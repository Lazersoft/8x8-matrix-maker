...
public class Matrixi2ccollab extends PApplet {
...
final static int ON  = 0xffFF0000, OFF = 0350, BG = 0; //main grid on/off colors (red)
...
float playy=6.5f*DIM;
float scrollx=DIM/6;
float scrollx2=9.78f*DIM;
float scrolly=8.75f*DIM;
float slidex1=GRID*DIM + DIM/3.4f;
...
public void draw() {
 ...
 fill(204);
  ...
  fill(0,255,0);
  triangle(playx+2*DIM/3,playy+3*DIM/4,playx+2*DIM/3,playy+DIM/4,playx+4.5f*DIM/3,playy+DIM/2);
  fill(150);
 ...
 /* for(int i=0;i<GRID;i++) //little row fillers surrounding matrix
  ...
  fill(0);
 // textSize(20);
  text("Frame",GRID*DIM+3.15f*GRID, DIM*4.15f);
  text("Clear",GRID*DIM+3.4f*GRID, 7*DIM/6);
  text("Fill",GRID*DIM+4.45f*GRID, DIM*2.65f);
  text("\n"+"Frame:" + count,GRID*DIM+GRID*1.5f, GRID*DIM-DIM*.5f);
  text("Save",GRID*DIM+3.9f*GRID, DIM*5.65f);
  //textSize(17);
...
if(leds[count][row][col] != 0){
    ...
    if(count<6){
      fill(0,255,0);
      rect(4.2f*DIM/5+1.8f*DIM*(count-1)-4,DIM*8.2f+5,DIM*1.8f,DIM*1.8f);
  for(int i=0;i<5;i++){
  for (int row = 0; row != GRID; ++row)
    for (int col = 0; col != GRID; ++col) {
...
if(leds2[i][row][col] != 0){
     ...
     fill(110);
      ...
      rect(col*DIM/5+4.2f*DIM/5+i*DIM*1.8f, row*DIM/5+8.425f*DIM, DIM/5, DIM/5);
}
  }
    if(saved==true){
    fill(0,0,255);
  text("Saved! Quit and copy Matrix.txt into your loop()!!!",DIM/8,height-DIM/10);//,width/2.2,height-DIM/10);
                    ...
                    fill(0,255,0);
      rect(4.2f*DIM/5+1.8f*DIM*(3)-4,DIM*8.2f+5,DIM*1.8f,DIM*1.8f);
        for(int i=0;i<5;i++){
  for (int row = 0; row != GRID; ++row)
    for (int col = 0; col != GRID; ++col) {
...
public void highlight(){
   for(int i=0;i<count;i++){
  if(mouseX>(4.2f*DIM/5+i*DIM*1.8f) && mouseX<(4.2f*DIM/5+i*DIM*1.8f)+DIM/5*8 && mouseY>(8.425f*DIM) && mouseY<(8.425f*DIM)+DIM/5*8 && mousePressed){
    hlight=true;
    j=i;
    break;
  }
    if(hlight==true){
    fill(0,255,0);
  rect(4.2f*DIM/5+1.8f*DIM*i-4,DIM*8.2f+5,DIM*1.8f,DIM*1.8f);
  //fill(0);
   //rect(DIM/5+i*DIM*1.8,8.2*DIM,DIM/5*8,DIM/5*8);
   for (int row = 0; row != GRID; ++row)
    for (int col = 0; col != GRID-1; ++col) {
    ...
    if(mouseX>(DIM/5+(count-1)*DIM*1.8f) && mouseX<(DIM/5+(count-1)*DIM*1.8f)+DIM/5*8 && mouseY>(8.2f*DIM) && mouseY<(8.2f*DIM)+DIM/5*8 && mousePressed){
        for (int row = 0; row != GRID; ++row)
    for (int col = 0; col != GRID; ++col) {
      leds[count][row][col]=0;
    }
...
float redx=GRID*DIM+DIM*2.6f;
float redy=DIM/3;
float redw=DIM/1.4f;
float redh=DIM/2.5f;
float greenx=GRID*DIM+DIM*3.4f;
float bluex=GRID*DIM+DIM*4.2f;
float colorx=GRID*DIM+DIM*2.6f;
float colory=DIM*.8f;
float colorw=DIM*2.3f;
float colorh=DIM/3;
...
fill(0); 
if(redbox==false)
 text(redVal,redx+DIM/25,redy+DIM/2.8f);
 if(greenbox==false)
 text(greenVal,greenx+DIM/25,redy+DIM/2.8f);
 if(bluebox==false)
 text(blueVal,bluex+DIM/25,redy+DIM/2.8f);
 //textSize(17);
...
public void redboxxer(){
     if(colorcounter==0)
      if(key=='1' || key=='2' || key=='0')
      rValtemp=100*(PApplet.parseByte(key)-48);
      else{}else if(colorcounter==1 && PApplet.parseByte(key) != 10)
    rValtemp+= (PApplet.parseByte(key)-48)*10;
    else if (colorcounter==2 && PApplet.parseByte(key) != 10)
    rValtemp+=PApplet.parseByte(key)-48;
    if(rValtemp<300 && rValtemp>0 && PApplet.parseByte(key) != 10){
    fill(0);
    text(key,redx+DIM/25+DIM/6*colorcounter,redy+DIM/3.5f);
    }
    colorcounter++;
    if(colorcounter==3 || PApplet.parseByte(key) == 10){
      colorcounter=0;
      redVal=rValtemp;
      redbox=false;
      rValtemp=0;
...
public void greenboxxer(){
       if(colorcounter==0)
      if(key=='1' || key=='2' || key=='0')
      rValtemp=100*(PApplet.parseByte(key)-48);
      else{}else if(colorcounter==1)
    rValtemp+= (PApplet.parseByte(key)-48)*10;
    else if (colorcounter==2)
    rValtemp+=PApplet.parseByte(key)-48;
    if(rValtemp<300 && rValtemp>0){
    fill(0);
    text(key,greenx+DIM/25+DIM/6*colorcounter,redy+DIM/3.5f);
    }
    colorcounter++;
    if(colorcounter==3){
      colorcounter=0;
...
public void blueboxxer(){
    ...
    if(rValtemp<300 && rValtemp>0){
    fill(0);
    text(key,bluex+DIM/25+DIM/6*colorcounter,redy+DIM/3.5f);
    }
    colorcounter++;
    if(colorcounter==3){
      colorcounter=0;
...
public void boxCheck(){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Matrixi2ccollab" };
    if (passedArgs != null) {
